...
int batPin = 35; // 电池电压采样所在引脚
float batVoltage = 0.0;
float batCompensate = 0.66;  //补偿的是二极管的压降,不同板子二极管压降可能不会完全一致,需要手动测量后对比偏差定义这个值
esp_adc_cal_characteristics_t *adc_chars;   // 声明校准特性结构体指针为全局变量
...
void myBat(lv_timer_t * timer) {
  batVoltage = analogRead(35)/4095.0*3.3*3.0+batCompensate;   
  Serial.println(analogRead(35));
  lv_span_set_text(screen_spangroup_1_span, (String(batVoltage)+"V").c_str());
}
...
void setup(void)
{
  ...
  delay(100);
  lv_timer_create(myBat, 2000, NULL);   //定时2S执行一个任务
